...
//[ ***step 1*** ] 初始化和其它关键帧的res(点在其它关键帧上投影)
    ...
    if(point->idepth_fromSensor > 0)
    {
	    currentIdepth = point->idepth_fromSensor;
	}
	else
	{
		currentIdepth = (point->idepth_max + point->idepth_min)*0.5f;
		//std::cout << std::endl << "point->depth_min = " << 1 / point->idepth_max << std::endl;
		//std::cout << "point->depth_max = " << 1 / point->idepth_min << std::endl;
	}
	// 2019.12.04 yzk
