...
{
	gradHistFrame = fh;			//代表为当前的fh生成梯度直方图
	...
	for(int y=0;y<h32;y++)
		for(int x=0;x<w32;x++)
		{
			...
			// 得到每一block的阈值
			ths[x+y*w32] = computeHistQuantil(hist0,setting_minGradHistCut) + setting_minGradHistAdd;			//setting: 0.5/7
		}
		...
		float* map_out, int pot, float thFactor)
{
	...
	{	
		...
		int my3 = std::min((4*pot), h-y4);		//40个12
		int mx3 = std::min((4*pot), w-x4);		//57个12, 剩下一个1
		int bestIdx4=-1; float bestVal4=0;
		// 随机系数
		Vec2f dir4 = directions[randomPattern[n2] & 0xF]; // 取低4位, 0-15, 和directions对应		//randomPattern 是unsigned char 的随机种子取到的值
		...
		for(int y3=0;y3<my3;y3+=(2*pot)) for(int x3=0;x3<mx3;x3+=(2*pot))
		{
			...
			int y34 = y3+y4;
			// 继续确定该层上的邻域
			int my2 = std::min((2*pot), h-y34); 	//40*2个6
			int mx2 = std::min((2*pot), w-x34);	//总体来说 57*2个6, 剩下一个1
			int bestIdx3=-1; float bestVal3=0;
			Vec2f dir3 = directions[randomPattern[n2] & 0xF];  
			//* 上面的邻域范围内, 变换到第0层, 每隔pot遍历
			//! 每个pot大小格里面一个大于阈值的最大的像素
			for(int y2=0;y2<my2;y2+=pot) for(int x2=0;x2<mx2;x2+=pot)		//第1层每个块中包含了4个第0层的pot
			{
				int x234 = x2+x34; // 坐标
				int y234 = y2+y34;
				int my1 = std::min(pot, h-y234);	//40*2*2个 3
				int mx1 = std::min(pot, w-x234);	//57*2*2个 3 剩下一个1
				...
				for(int y1=0;y1<my1;y1+=1) for(int x1=0;x1<mx1;x1+=1)
				{
					...
					if(ag0 > pixelTH0*thFactor)
					{
						...
						// 2019.12.11 yzk
						/*
						float graDepthX1 = fabs(fh->dDepth[0][w * yf + xf - 1] - fh->dDepth[0][w * yf + xf + 1]) / 2;
						float graDepthX3 = fabs(fh->dDepth[0][w * yf + xf - 3] - fh->dDepth[0][w * yf + xf + 3]) / 6;

						float graDepthY1 = fabs(fh->dDepth[0][w * yf + xf - w] - fh->dDepth[0][w * yf + xf + w]) / 2;
						float graDepthY3 = fabs(fh->dDepth[0][w * yf + xf - 3 * w] - fh->dDepth[0][w * yf + xf + 3 * w]) / 6;

						bool flag1 = (graDepthX1 < 3 * graDepthX3) && (graDepthX3 < 3 * graDepthX1);
						bool flag2 = (graDepthY1 < 3 * graDepthY3) && (graDepthY3 < 3 * graDepthY1);
						bool flag3 = graDepthX1 > 0 && graDepthX3 > 0 && graDepthY1 > 0 && graDepthY3 > 0;
						bool flagAll = flag1 && flag2 && flag3;
						*/
						// 2019.12.11 yzk
