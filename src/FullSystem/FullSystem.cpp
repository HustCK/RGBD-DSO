...
//[ ***step 2*** ] 处理未成熟点, 激活/删除/跳过
	for(FrameHessian* host : frameHessians)		// go through all active frames
	{
		...
		for(unsigned int i=0;i<host->immaturePoints.size();i+=1)
		{
	        ...
	        SE3 cameraToWorld = host->shell->camToWorld;
	        Mat33f Rwc = (cameraToWorld.rotationMatrix()).cast<float>();
	        Vec3f twc = (cameraToWorld.translation()).cast<float>();

	        Vec3f pr = K * (Rwc * Vec3f(ph->u, ph->v, 1) * ph->idepth_fromSensor + twc);
	        ...
	        */
			...
			bool canActivate = (ph->lastTraceStatus == IPS_GOOD
					...
					|| ph->lastTraceStatus == IPS_OOB )
							&& ph->lastTracePixelInterval < 8
							&& ph->quality > setting_minTraceQuality
							&& (ph->idepth_max+ph->idepth_min) > 0 && ph->idepth_fromSensor > 0;
			*/
			// 2019.12.06 yzk
 ...
 *******************************/
void FullSystem::addActiveFrame( ImageAndExposure* image, ImageAndExposure* depthImage, int id )
{
	//[ ***step 1*** ] track线程锁
    if(isLost) return;
	boost::unique_lock<boost::mutex> lock(trackMutex);
